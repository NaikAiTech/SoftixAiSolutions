...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  const url = `${siteBase}/vet-answers/${encodeURIComponent(slug)}`;
...
export default async function VetAnswerDetailPage({ params }: { params: { slug: string } }) {
  ...
  const siteBase = (process.env.NEXT_PUBLIC_SITE_URL || process.env.NEXTAUTH_URL || 'https://www.dialavet.com').replace(/\/$/, '');
  const canonicalUrl = `${siteBase}/vet-answers/${encodeURIComponent(item.slug)}`;
  const qaLd: any = {
  ...
  return (
    <main className="mx-auto w-full max-w-3xl bg-[#f7f7f8] px-4 py-8 lg:py-10">
      ...
      <section className="mb-4 space-y-4">
        <FreeConsultBanner source={`vet-answers:${item.slug || ""}`} />
        <div>
