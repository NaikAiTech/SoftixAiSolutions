...
export default async function AskAVetListPage() {
  const base = process.env.NEXTAUTH_URL || "";
  const res = await fetch(`${base}/api/ask-a-vet/internal/list?limit=9&page=1`, { cache: "no-store" }).catch(() => null);
  ...
  return (
    <main className="container" style={{ paddingTop: 24, paddingBottom: 28 }}>
      <header className="card surface" style={{ padding: 20, borderRadius: 20, marginBottom: 16 }}>
        <div className="h1" style={{ marginBottom: 6 }}>Ask A Vet</div>
        <div className="lead">Browse common questions from pet owners and our team’s responses.</div>
      </header>

      <AskAVetSearch defaultItems={items} linkPrefix="/ask-a-vet" searchPath="/api/ask-a-vet/internal/list" initialPage={initialPage} initialPerPage={initialPerPage} initialTotal={initialTotal} />
